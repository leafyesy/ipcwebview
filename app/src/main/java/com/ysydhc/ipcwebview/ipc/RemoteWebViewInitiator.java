...
public class RemoteWebViewInitiator implements IIPCInitiatorTask {
    ...
    public void init(BinderManager manager) {
        ...
        InterfaceIpcHub.getInstance().setConnectObjectCreateList(cell -> {
            if (cell != null && cell.getArguments() != null) {
                ...
                if (flagRemoteWeb == Boolean.TRUE) {
                    InterfaceProxy<?> interfaceProxy = InterfaceIpcHub.getInstance().fetchCallObject(cell.getKey());
                    if (interfaceProxy != null && interfaceProxy.getOutProxy() != null) {
                        return interfaceProxy.getOutProxy();
                    }
